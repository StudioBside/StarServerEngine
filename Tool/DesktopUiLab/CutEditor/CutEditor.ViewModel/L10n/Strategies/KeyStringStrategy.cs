...
internal sealed class KeyStringStrategy(VmL10n viewModel) : L10nStrategyBase(L10nSourceType.KeyString)
{
    ...
    public override bool ImportFile(string fileFullPath, ISet<string> importedHeaders)
    {
        ...
        {
            if (dicMappings.TryGetValue(imported.PrimeKey, out var mapping) == false)
            {
    ...
    public override bool SaveToFile(string name, L10nType l10nType)
    {
        ...
        var dbName = L10nReadOnlyDb.KeyStringDbName;
        ...
        using (writer)
        {
            foreach (var mapping in this.mappings)
            {
                ...
                writer.Upsert(mapping.UidStr, l10nType, newValue);
            }
        }
